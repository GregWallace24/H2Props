#Title: RelEnthDiffPlots.py
#Author: Greg Wallace
#Organization: Washington State University, HYPER Lab
#Date Created: September 6, 2021
#Date Last Edited: September 6, 2021
#Purpose: Plot the difference in relative enthalpies of orthohydrogen, normal hydrogen, and equilibrium hydrogen compared to parahydrogen.
...
for i in range(15,300+1):#Properties of hydrogen do not go below 14K, Room temp is about 300K
    ...
    ReldH100.append(h_mix(i,P,1.0)/1000-h_mix(i,P,0.0)/1000)#convert to kJ/kg, orthohydrogen - parahydrogen
    ReldH75.append(h_mix(i,P,0.75)/1000-h_mix(i,P,0.0)/1000)#convert to kJ/kg, normal hydrogen - parahydrogen
    ReldHeq.append(h_mix(i,P,Yo_equilib(i))/1000-h_mix(i,P,0.0)/1000)#convert to kJ/kg, equilibrium hydrogen - parahydrogen
...
ReldHPlot.plot(T,ReldH100,color='red')#plot enthalpy difference between orthohydrogen and parahydrogen
ReldHPlot.plot(T,ReldH75,color='green')#plot enthalpy difference between normal hydrogen and parahydrogen
ReldHPlot.plot(T,ReldHeq,color='black')#plot enthalpy difference between equilibrium hydrogen and parahydrogen
ReldHPlot.set_xlabel('Temperature [K]')
ReldHPlot.set_ylabel('Relative Enthalpy Difference from Parahydrogen [kJ/kg]')
ReldHPlot.set_xticks(np.arange(0, 300+20, 20))
ReldHPlot.set_yticks(np.arange(0, 750+50, 50))
ReldHPlot.set_xlim([0,300])
ReldHPlot.set_ylim([0,751])
ReldHPlot.grid(True)
ReldHPlot.legend(['Orthohydrogen','Normal Hydrogen','Equilibrium'])
plt.show()
